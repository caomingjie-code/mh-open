 ...
 * @createTime 2021年06月20日 21:09:00
 */
...
public @interface EnableKafkaProducer {
     ...
     */
     String compressionType()default "";
     ...
     */
     String maxInFlightRequestsPerConnection()default "";


    /**
     * 指定了broker等待同步副本返回消息的确认消息，与acks相匹配，如果在指定的时间内没有收到确认消息则会报错。
     */
     String timeoutMs()default "";

    /**
     * 指生产者在发送数据时等待服务器返回的时间。
     */
     String requestTimeoutMs()default "";

    /**
     * 指生产者在获取元数据（获取目标分区的首领是谁）时等待服务器的时间。
     */
     String metadataFatchTimeoutMs()default "";
     ...
     */
     String receiveBufferBytes()default "";
     String sendBufferBytes()default "";


    /**
     * 实现了org.apache.kafka.clients.producer.的分区器类。分区程序界面。可以自定义分区策略
     */
     String partitionerClass()default "";
